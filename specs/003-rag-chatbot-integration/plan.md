...
Integrate a fully functional RAG chatbot within the Docusaurus book, which can answer any user question based on the book content, including selected text, and provide citations. This will be achieved using a FastAPI backend, Qdrant for vector storage, Neon PostgreSQL for chat history, and a Docusaurus/React frontend with Google Gemini as the LLM.

## Technical Context

**Language/Version**: Python 3.11+ (FastAPI), TypeScript (React/Docusaurus)
**Primary Dependencies**: FastAPI, Google AI SDK, Qdrant Client, Neon psycopg2, React, Docusaurus, Tailwind CSS
